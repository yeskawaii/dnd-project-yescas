...
const AddSpellModal = ({ isOpen, onClose, onAdd }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
      <div className="bg-slate-900 border border-cyan-500/30 w-full max-w-sm rounded-3xl p-8 shadow-[0_0_50px_rgba(6,182,212,0.1)] animate-in zoom-in duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-800 p-3 rounded-xl text-white outline-none font-bold"
              />
            </div>
            <div>
              ...
              <select 
                ...
                className="w-full bg-slate-800 p-3 rounded-xl text-white outline-none font-bold appearance-none cursor-pointer"
              >
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-slate-800 p-3 rounded-xl text-white outline-none focus:ring-2 ring-cyan-500 h-24 text-xs font-medium italic"
              placeholder="Ej: 1d6 de daño por nivel..."
            />
          </div>

          <div className="flex gap-3 pt-4">
            <button type="button" onClick={onClose} className="flex-1 p-3 rounded-xl text-slate-500 font-black text-[10px] hover:bg-slate-800 transition-all">CANCELAR</button>
            <button type="submit" className="flex-1 bg-cyan-600 p-3 rounded-xl text-white font-black text-[10px] shadow-lg shadow-cyan-900/40 hover:bg-cyan-500 transition-all uppercase">Aprender</button>
          </div>
        </form>
      </div>
    </div>
