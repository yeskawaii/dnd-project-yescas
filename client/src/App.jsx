...
function App() {
  ...
  const handleFinalUpdate = (newValue) => {
    const { type, statName } = editModal;
    let val = newValue;

    // Conversiones métricas
    if (unitSystem === "metric") {
      if (["physicalWeight", "weightMax"].includes(type)) val = Number(kgToLb(val));
      else if (type === "height") val = Number(cmToIn(val));
    }

    // 1. Mapeo de campos simples (BAB, Bio, etc.)
    const simpleFields = {
      bab: "baseAttack",
      init_misc: "initiativeMisc",
      name: "name",
      class: "class",
      race: "race",
      level: "level",
    ...
    // 2. Mapeo de objetos complejos
    const complexMappings = {
      stat: { stats: { ...selectedChar.stats, [statName]: Number(val) } },
      hp: { hp: { ...selectedChar.hp, current: Number(val) } },
      maxHp: { hp: { ...selectedChar.hp, max: Number(val) } },
      ca_armor: { armorClass: { ...selectedChar.armorClass, armor: Number(val) } },
      weightMax: { weight: { ...selectedChar.weight, max: Number(val) } },
      cp: { money: { ...selectedChar.money, cp: Number(val) } },
      sp: { money: { ...selectedChar.money, sp: Number(val) } },
      gp: { money: { ...selectedChar.money, gp: Number(val) } },
      pp: { money: { ...selectedChar.money, pp: Number(val) } }
    };

    // 3. Salvaciones Dinámicas
    if (type.startsWith("save_")) {
      const saveKey = type.split("_")[1];
      return handleUpdateCharacter({ saves: { ...selectedChar.saves, [saveKey]: Number(val) } });
    }

    // 4. Lógica de Notas (Post/Patch independiente)
    if (type === "note" || type === "edit_note") {
      const isEdit = type === "edit_note";
      const url = isEdit ? `/character/${selectedChar._id}/notes/${statName}` : `/character/${selectedChar._id}/notes`;
      api[isEdit ? 'patch' : 'post'](url, isEdit ? { content: val } : { title: "Diario", content: val, color: "cyan" })
        .then((res) => {
          const updatedChar = { 
            ...selectedChar, 
            notes: isEdit ? selectedChar.notes.map(n => n._id === statName ? res.data : n) : [...(selectedChar.notes || []), res.data] 
          };
    ...
    // Ejecución final
    const dbKey = simpleFields[type] || type;
    const updateData = complexMappings[type] || { [dbKey]: isNaN(val) ? val : Number(val) };
  ...
  const handleLeaveCampaign = (characterId) => {
    askDelete("Abandonar", "¿Dejar partida?", () => {
      api.post("/campaign/leave", { characterId }).then(() => {
        toast.success("Libre");
  ...
  if (loading) return <div className="h-screen bg-slate-950 flex items-center justify-center text-cyan-500 font-black animate-pulse">CARGANDO...</div>;
        ...
        <>
          {!selectedChar ? (
            <div className="min-h-screen p-8 flex flex-col items-center">
              <button onClick={() => setMode("lobby")} className="text-cyan-500 text-[10px] mb-6 tracking-widest uppercase">← VOLVER AL LOBBY</button>
              <h1 className="text-orange-500 font-black text-3xl mb-12 italic">Aventureros</h1>
              <div className="grid gap-4 w-full max-w-sm">
                {characters.map((c) => (
                  <motion.div whileTap={{ scale: 0.95 }} key={c._id} className="rounded-2xl bg-slate-900 border border-slate-800 p-4 flex items-center justify-between shadow-lg">
                    <button onClick={() => setSelectedChar(c)} className="flex-1 text-left truncate">
                      <div className="flex items-center gap-2 mb-1">
                        <h2 className="text-white font-black text-lg truncate leading-none">{c.name}</h2>
                        {c.campaign && <span className="bg-orange-500/10 text-orange-500 text-[7px] px-2 py-0.5 font-black border border-orange-500/30 rounded-sm">🗡️ {c.campaign.name}</span>}
                      </div>
                      <p className="text-slate-500 text-[10px] font-bold tracking-widest">{c.class} • LVL {c.level}</p>
                    </button>
                    <button onClick={() => askDelete("Borrar", "¿Eliminar héroe?", () => api.delete(`/character/${c._id}`).then(() => setCharacters(characters.filter(char => char._id !== c._id))))} className="w-10 h-10 bg-red-500/10 text-red-500 rounded-xl">🗑️</button>
                  </motion.div>
                ))}
                <button onClick={() => setIsJoinModalOpen(true)} className="w-full border-2 border-cyan-900/30 p-4 rounded-2xl text-cyan-600 font-black text-xs mt-4 italic">🔗 UNIRSE A CONTIENDA</button>
                <button onClick={() => setIsCharModalOpen(true)} className="w-full border-2 border-dashed border-slate-800 p-4 rounded-2xl text-slate-600 font-black text-xs mt-2">+ CREAR NUEVO</button>
              </div>
            </div>
          ) : (
            <div className="flex flex-col min-h-screen pb-24">
              <header className="px-6 py-4 pt-10 sticky top-0 z-50 bg-slate-950 border-b border-slate-800 flex justify-between items-center shadow-md">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 onClick={() => openEdit("Nombre", "NOMBRE", selectedChar.name, "name")} className="text-xl font-black text-white active:text-orange-500 italic tracking-tighter leading-none">{selectedChar.name} ✎</h1>
                    <div className="flex items-center gap-2 mt-1">
                      <p onClick={() => openEdit("Nivel", "NIVEL", selectedChar.level, "level")} className="text-[9px] font-black text-slate-500 tracking-widest cursor-pointer active:text-white">{selectedChar.class} LVL {selectedChar.level} ✎</p>
                      <span onClick={() => openEdit("Experiencia", "XP TOTAL", selectedChar.experience, "experience")} className="text-[9px] font-black text-cyan-600 cursor-pointer active:text-cyan-400">XP: {selectedChar.experience || 0} ✎</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="max-w-5xl mx-auto w-full px-4 mt-6">
                <nav className="hidden md:flex justify-center gap-2 mb-8 bg-slate-900/50 p-2 rounded-2xl border border-slate-800 w-max mx-auto shadow-lg backdrop-blur-sm">
                  {[{ id: "stats", icon: "⚔️", label: "Stats" }, { id: "inv", icon: "🎒", label: "Mochila" }, { id: "spells", icon: "🪄", label: "Spells" }, { id: "notes", icon: "📜", label: "Notas" }].map((tab) => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`relative flex items-center gap-2 px-6 py-3 rounded-xl font-black text-xs ${activeTab === tab.id ? "text-cyan-400 bg-cyan-500/10" : "text-slate-500 active:text-slate-300 active:bg-slate-800/50"}`}>
                      <span className="text-lg z-10">{tab.icon}</span><span className="z-10">{tab.label}</span>
                    ...
                    <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start mt-6">
                            <CollapsibleSection title="Biografía">
                              <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                                <button onClick={() => setUnitSystem(unitSystem === "imp" ? "metric" : "imp")} className="bg-orange-600/20 border border-orange-500/40 px-3 py-2 rounded-2xl text-[8px] font-black text-orange-500 flex-shrink-0 active:bg-orange-600/30">UNIDADES: {unitSystem === "imp" ? "LB/FT" : "KG/CM"}</button>
                                {["race", "alignment", "deity"].map((f) => (
                                  <div key={f} onClick={() => openEdit(`Editar ${f}`, f.toUpperCase(), selectedChar[f], f)} className="bg-slate-900/50 border border-slate-800 px-4 py-2 rounded-2xl flex-shrink-0 cursor-pointer active:bg-slate-800">
                                ...
                                <div onClick={() => openEdit("Altura", "ESTATURA", selectedChar.height, "height")} className="bg-slate-900/50 border border-slate-800 px-4 py-2 rounded-2xl flex-shrink-0 cursor-pointer">
                                  <p className="text-[7px] font-black text-slate-600 tracking-tighter">Altura</p>
                                ...
                                <div onClick={() => openEdit("Peso Corporal", "SIN EQUIPO", selectedChar.physicalWeight, "physicalWeight")} className="bg-slate-900/50 border border-slate-800 px-4 py-2 rounded-2xl flex-shrink-0 cursor-pointer">
                                  <p className="text-[7px] font-black text-slate-600 tracking-tighter">Peso Pers.</p>
                            ...
                            <CollapsibleSection title="Combate" defaultOpen={true}>
                              <div className="grid grid-cols-3 gap-3 mb-4 text-center">
                                <div onClick={() => openEdit("Armadura", "EQUIPO", selectedChar.armorClass?.armor, "ca_armor")} className="bg-slate-900 border border-cyan-500/20 p-3 rounded-3xl cursor-pointer">
                                  <p className="text-[8px] font-black text-cyan-400 mb-1 tracking-widest uppercase">AC</p>
                                  <span className="text-3xl font-black text-white">{totalCA}</span>
                                </div>
                                <div onClick={() => openEdit("BAB", "BASE", selectedChar.baseAttack, "bab")} className="bg-slate-900 border border-orange-500/20 p-3 rounded-3xl cursor-pointer">
                                  <p className="text-[8px] font-black text-orange-400 mb-1 tracking-widest">BAB</p>
                                  <span className="text-2xl font-black text-white">+{selectedChar.baseAttack || 0}</span>
                                </div>
                                <div onClick={() => openEdit("Iniciativa", "MISC", selectedChar.initiativeMisc, "init_misc")} className="bg-slate-900 border border-purple-500/20 p-3 rounded-3xl cursor-pointer">
                                  <p className="text-[8px] font-black text-purple-400 mb-1 tracking-widest uppercase">Iniciativa</p>
                              ...
                              <div className="mt-6 pt-4 border-t border-slate-800">
                                <div className="flex justify-between items-center mb-3"><h3 className="text-[10px] font-black text-slate-500 uppercase italic tracking-widest">Armas Equipadas</h3><button onClick={() => setIsAttackModalOpen(true)} className="text-[9px] font-black text-orange-500 bg-orange-500/10 px-2 py-1 rounded">+ AGREGAR</button></div>
                                <div className="space-y-2">{selectedChar.attacks?.map((atk) => <AttackCard key={atk._id} attack={atk} character={selectedChar} onDelete={(id) => api.delete(`/character/${selectedChar._id}/attacks/${id}`).then(() => handleUpdateCharacter({ attacks: selectedChar.attacks.filter(a => a._id !== id) }))} />)}</div>
                              </div>
                            </CollapsibleSection>
                            ...
                            <CollapsibleSection title="Habilidades (Skills)">
                              <div className="mt-2 md:max-h-[500px] overflow-y-auto custom-scrollbar">
                            ...
                            <CollapsibleSection title="Rasgos y Dotes">
                              <div className="space-y-4 mt-2">
                                <div>
                                  <div className="flex justify-between mb-2"><span className="text-[8px] font-black text-slate-500 uppercase italic">Innatas</span><button onClick={() => setIsTraitModalOpen(true)} className="text-[9px] font-black text-blue-500 uppercase tracking-widest">+ AGREGAR</button></div>
                                  {selectedChar.traits?.map(t => <TraitCard key={t._id} trait={t} onDelete={(id) => handleUpdateCharacter({ traits: selectedChar.traits.filter(x => x._id !== id) })} />)}
                                </div>
                                <div>
                                  <div className="flex justify-between mb-2"><span className="text-[8px] font-black text-slate-500 uppercase italic">Dotes</span><button onClick={() => setIsFeatModalOpen(true)} className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">+ AGREGAR</button></div>
                                  {selectedChar.feats?.map(f => <FeatCard key={f._id} feat={f} onDelete={(id) => api.delete(`/character/${selectedChar._id}/feats/${id}`).then(() => handleUpdateCharacter({ feats: selectedChar.feats.filter(x => x._id !== id) }))} />)}
                                </div>
                              </div>
                            </CollapsibleSection>
                          </div>
                          <div className="mt-8 px-2 pb-10">
                            <div className="flex justify-between items-end mb-2">
                              <span className="text-[8px] font-black text-slate-600 tracking-widest uppercase italic">Carga Mochila ({unitSystem === "imp" ? `${totalWeight} lb` : `${lbToKg(totalWeight)} kg`})</span>
                              <span onClick={() => openEdit("Máximo", "CARGA MAX", selectedChar.weight?.max, "weightMax")} className="text-[10px] font-black text-white cursor-pointer">MAX: {unitSystem === "imp" ? `${selectedChar.weight?.max || 100} LB` : `${lbToKg(selectedChar.weight?.max || 100)} KG`}</span>
                            </div>
                      ...
                      {activeTab === "inv" && (
                        <div className="space-y-6">
                          ...
                          <div>
                            <h2 className="text-2xl font-black text-white mb-4 italic tracking-tighter uppercase">Mochila</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                              {selectedChar.inventory?.map((i) => <InventoryItem key={i._id} item={i} onDelete={(id) => api.delete(`/character/${selectedChar._id}/inventory/${id}`).then(() => handleUpdateCharacter({ inventory: selectedChar.inventory.filter(x => x._id !== id) }))} onEdit={() => { setEditingItem(i); setIsModalOpen(true); }} />)}
                            </div>
                            <button onClick={() => { setEditingItem(null); setIsModalOpen(true); }} className="w-full mt-6 border-2 border-dashed border-slate-800 p-4 rounded-2xl text-slate-600 font-black text-[10px] uppercase shadow-xl tracking-widest">+ AGREGAR OBJETO</button>
                          </div>
                        </div>
                      )}
                      ...
                      {activeTab === "spells" && (
                        <div className="pb-10">
                          <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">Grimorio</h2>
                            <button onClick={() => { setEditingSpell(null); setIsSpellModalOpen(true); }} className="bg-cyan-600 px-4 py-1.5 rounded-full text-[10px] font-black active:bg-cyan-500 shadow-lg tracking-widest uppercase">+ APRENDER</button>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {selectedChar.spells?.sort((a,b) => a.level - b.level).map((s) => (
                              <SpellCard key={s._id} spell={s} onToggle={(id) => api.patch(`/character/${selectedChar._id}/spells/${id}`).then(r => handleUpdateCharacter({ spells: selectedChar.spells.map(x => x._id === id ? r.data : x) }))} onDelete={(id) => handleUpdateCharacter({ spells: selectedChar.spells.filter(x => x._id !== id) })} onEdit={() => { setEditingSpell(s); setIsSpellModalOpen(true); }} />
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {activeTab === "notes" && (
                        <div className="pb-10">
                          <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">Diario</h2>
                            <button onClick={() => openEdit("Nota", "DIARIO", "", "note")} className="bg-slate-800 border border-slate-700 px-4 py-1.5 rounded-full text-[10px] font-black active:bg-slate-700 shadow-lg uppercase tracking-widest">+ ESCRIBIR</button>
                          </div>
                          <div className="columns-1 md:columns-2 gap-4">
                            {selectedChar.notes?.map((n) => <NoteCard key={n._id} note={n} onDelete={(id) => api.delete(`/character/${selectedChar._id}/notes/${id}`).then(() => handleUpdateCharacter({ notes: selectedChar.notes.filter(x => x._id !== id) }))} onEdit={() => openEdit("Editar Nota", "DIARIO", n.content, "edit_note", n._id)} />)}
                          </div>
                        </div>
                      )}
                    </motion.div>
