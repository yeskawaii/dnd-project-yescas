...
import AttackCard from "./components/AttackCard"; // <-- NUEVO
import AddAttackModal from "./components/AddAttackModal"; // <-- NUEVO
...
function App() {
  ...
  const [isAttackModalOpen, setIsAttackModalOpen] = useState(false); // <-- NUEVO
  ...
  const handleFinalUpdate = (newValue) => {
    ...
    switch (t) {
      case 'name': updateData = { name: valToSave }; break; // EDITAR NOMBRE
      case 'class': updateData = { class: valToSave }; break; // EDITAR CLASE
  ...
  // HANDLERS DE ARMAS Y ATAQUES <-- NUEVO
  const handleAddAttack = (a) => api.post(`/character/${selectedChar._id}/attacks`, a).then(r => setSelectedChar({...selectedChar, attacks: [...(selectedChar.attacks || []), r.data]}));
  const handleDeleteAttack = (id) => api.delete(`/character/${selectedChar._id}/attacks/${id}`).then(() => setSelectedChar({...selectedChar, attacks: (selectedChar.attacks || []).filter(a => a._id !== id)}));
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-8 flex flex-col items-center">
        ...
        <div className="grid gap-4 w-full max-w-sm">
          {characters.map((c) => (
            <motion.div whileTap={{ scale: 0.95 }} key={c._id} className="rounded-2xl bg-slate-900 border border-slate-800 p-6 active:scale-95 transition-all">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-32 font-sans uppercase">
        ...
        <div className="flex items-center gap-3 w-full">
          ...
          <div className="flex flex-col w-full overflow-hidden">
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              <span 
                onClick={() => openEdit("Clase", "PROFESIÓN", selectedChar.class, 'class')}
              ...
              <span 
                onClick={() => openEdit("Experiencia", "XP TOTAL", selectedChar.experience || 0, 'experience')}
      ...
      <main className="p-4 max-w-md mx-auto">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === "spells" && (
              <div><div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black text-white italic tracking-tighter">Grimorio</h2>
              <button onClick={() => setIsSpellModalOpen(true)} className="bg-cyan-600 px-4 py-1 rounded-full text-[10px] font-black hover:bg-cyan-500 transition-colors">+ APRENDER</button></div>
            ...
            {activeTab === "notes" && (
              <div><div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black text-white italic tracking-tighter">Diario</h2>
              <button onClick={() => openEdit("Nota", "ESCRIBIR EN EL DIARIO", "", "note")} className="bg-slate-800 border border-slate-700 px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-slate-700 transition-colors">+ ESCRIBIR</button></div>
      ...
      <AddAttackModal isOpen={isAttackModalOpen} onClose={() => setIsAttackModalOpen(false)} onAdd={handleAddAttack} /> {/* <-- NUEVO */}
